...
Box(3,)
>>> pendulumEnv.observation_space.low
array([-1., -1., -8.], dtype=float32)
>>> pendulumEnv.observation_space.high
array([1., 1., 8.], dtype=float32)
>>> pendulumEnv.action_space.low
array([-2.], dtype=float32)
>>> pendulumEnv.action_space.high
array([2.], dtype=float32)
"""
...
class NeuralNet(torch.nn.Module):
    def __init__(self, hiddenLayerWidths=(8,5)):
        ...
        for hiddenLayerNdx in range(len(hiddenLayerWidths) + 1):
            if hiddenLayerNdx == 0:
                numberOfInputs = 3 # pendulumEnv.observation_space
            else:
                numberOfInputs = hiddenLayerWidths[hiddenLayerNdx - 1]

            if hiddenLayerNdx == len(hiddenLayerWidths):
                numberOfOutputs = 1 # pendulumEnv.action_space, continuous
            else:
        ...
        self.observation_low = [-1., -1., -8.]
        self.observation_high = [1., 1., 8.]
        self.action_low = [-2.]
        self.action_high = [2.]
    ...
    def forward(self, inputs):
        ...
        return self.action_low[0] + torch.sigmoid(dataState) * (self.action_high[0] - self.action_low[0]) #torch.tanh(dataState) # Squashes the value in ]-1, 1[ . See action_space.low, action_space.high
...
def main():
    print ("Pendulum-v0/NNController.py main()")
